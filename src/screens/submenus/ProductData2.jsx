...
const ProductDetails = () => {
  ...
  const fetchTeam = async () => {
    ...
    const accessToken = localStorage.getItem("accessToken");
    try {
      const response = await instance.get("Product_data2/find", {
        headers: {
          Authorization: "Bearer " + accessToken,
          "Content-Type": "application/json",
        },
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateForm(formData)) {
      ...
      try {
        if (editMode) {
          await instance.put(`Product_data2/update/${editingId}`, data, {
            headers: {
              Authorization: "Bearer " + accessToken,
              "Content-Type": "application/json",
            },
          ...
          await instance.post("Product_data2/add", data, {
            headers: {
              Authorization: "Bearer " + accessToken,
              "Content-Type": "application/json",
            },
      ...
      buttons: [
        {
          ...
          onClick: async () => {
            const accessToken = localStorage.getItem("accessToken");
            try {
              await instance.delete(`Product_data2/isdelete/${id}`, {
                headers: {
                  Authorization: "Bearer " + accessToken,
                  "Content-Type": "application/json",
                },
  ...
  const handleIsActive = (id, isActive) => {
    ...
    instance
      .put(`Product_data2/isactive/${id}`, { isActive }, {
        headers: {
          Authorization: "Bearer " + accessToken,
          "Content-Type": "application/json",
        },
