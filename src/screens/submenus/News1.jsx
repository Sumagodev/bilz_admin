...
const Infrastructure = () => {
  ...
  const fetchTeam = async () => {
    ...
    try {
      const response = await instance.get("News1/find", {
        headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateForm(formData)) {
      ...
      try {
        if (editMode) {
          await instance.put(`News1/update/${editingId}`, data, {
            headers: {
              Authorization: "Bearer " + accessToken,
          ...
          await instance.post("News1/add", data, {
            headers: {
              Authorization: "Bearer " + accessToken,
              "Content-Type": "multipart/form-data",
            },
          ...
          >
            <button
              ...
              onClick={async () => {
                ...
                try {
                  await instance.delete(`News1/isdelete/${id}`, {
                    headers: {
                      Authorization: `Bearer ${accessToken}`,
                      "Content-Type": "application/json",
                    },
